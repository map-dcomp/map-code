set title "Region B RLG Overflow Plans: Cross Region Shedding"
...
if (system ("awk -F ',' '{sum += $6 + $7 + $8 + $9} END {print sum}' service_AppCoordinates_\{com.bbn__webservice__1\}-rlg_overflow_plans.csv") > 0) { \
set output "4-rlg-overflow-b.eps"; \
plot for [i=6:9] 'service_AppCoordinates_{com.bbn__webservice__1}-rlg_overflow_plans.csv' using 1:i dt i lt i lw 3 \
} else { \
system ("echo no") \
}
