set title "Region X RLG Overflow Plans: Cross Region Shedding"
...
if (system ("awk -F ',' '{sum += $14 + $15 + $16 + $17} END {print sum}' service_AppCoordinates_\{com.bbn__webservice__1\}-rlg_overflow_plans.csv") > 0) { \
set output "4-rlg-overflow-x.eps"; \
plot for [i=14:17] 'service_AppCoordinates_{com.bbn__webservice__1}-rlg_overflow_plans.csv' using 1:i dt i lt i lw 3 \
} else { \
system ("echo no") \
}
