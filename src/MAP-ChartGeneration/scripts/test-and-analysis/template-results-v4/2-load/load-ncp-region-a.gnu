set title "NCP Load Plot Region A - Container Count"
...
if (system ("awk -F ',' '{sum += $2 + $3 + $4 + $5 + $6} END {print sum}' nodes-TASK_CONTAINERS.csv") > 0) { \
set output "2c-load-nodes-a-TASK_CONTAINERS.eps"; \
plot for [i=2:6] 'nodes-TASK_CONTAINERS.csv' using 1:i dt i lt i \
} else { \
system ("echo no") \
}
