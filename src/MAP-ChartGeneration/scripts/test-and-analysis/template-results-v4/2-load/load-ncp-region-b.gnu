set title "NCP Load Plot Region B - Container Count"
...
if (system ("awk -F ',' '{sum += $7 + $8 + $9 + $10 + $11} END {print sum}' nodes-TASK_CONTAINERS.csv") > 0) { \
set output "2c-load-nodes-b-TASK_CONTAINERS.eps"; \
plot for [i=7:11] 'nodes-TASK_CONTAINERS.csv' using 1:i dt i lt i \
} else { \
system ("echo no") \
}
